...
//    ))
    ...
    def moveModel(): Unit ={
    import java.io.File
    val modelName = "randomForestRegressionModel"
    val url = getClass().getResource(modelName)
    val dir = new File(url.toURI)
    val tempDir = "F:/Apoorv/Fall2017/CS6240/project/temp/"
    val modelDir = new File(tempDir+modelName)
    modelDir.mkdir()
    iterateOverFiles(dir, modelDir.toString+"/")
  }

  def iterateOverFiles(modelDir: File, tempModelDir: String):Unit = {
    for (nextFile <- modelDir.listFiles) {
      if (nextFile.isDirectory) {
        val file = new File(tempModelDir+getPath(nextFile.toString, "dir"))
        file.mkdir()
        iterateOverFiles(nextFile, tempModelDir)
      }
      else {
        new FileOutputStream(new File(tempModelDir+getPath(nextFile.toString, "file"))) getChannel() transferFrom(
          new FileInputStream(nextFile) getChannel, 0, Long.MaxValue )
      }
    }
  }

  def getPath(p: String, i: String): String = {
    if(i.equals("dir")){
      val path = p.substring(p.lastIndexOf("\\") + 1)
      return path
    }
    else {
      val fileName = p.substring(p.lastIndexOf("\\") + 1)
      val remName = p.substring(0, p.lastIndexOf("\\"))
      val file = p.substring(remName.lastIndexOf("\\") + 1)
      println( file)
      return file
    }
  }
